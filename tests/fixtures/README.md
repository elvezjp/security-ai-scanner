...
- `sais-v1-0.3.0/` is generated through the real `sais` runner and an offline
  deterministic engine. It contains completed, incomplete, and error artifacts
  labeled `0.3.0` for downstream conformance tests.

The synthetic `qk` fixtures remain the normative executable boundary. The
version-pinned producer fixtures prove that the released implementation reaches
that boundary; they do not replace the hand-authored examples.
...
The files under `sais-v1-0.3.0/` are the producer-side handoff source for
`quality-keeper`. Their source commit is recorded downstream after the release
tag makes it immutable.
